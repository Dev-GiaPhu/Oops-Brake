...
namespace EmergencyRoad
{
    public sealed class EmergencyRoadGame : MonoBehaviour
    {
        ...
        private void BuildPlayer()
        {
            ...
            player = root.AddComponent<EmergencyVehicleController>(); player.Initialize(visual.transform, this);
    ...
    internal sealed class RoadChunk
    {
        ...
        private void SpawnCrossing()
        {
            ...
            var go=prefab?Object.Instantiate(prefab,root.transform):GameObject.CreatePrimitive(PrimitiveType.Cube);go.name="Cross Traffic Hazard";go.transform.localPosition=new Vector3(left?-12f:12f,.45f,8f);go.transform.localRotation=Quaternion.Euler(0,left?90:-90,0);EmergencyRoadGame.FitVehicle(go,2.1f,4f);go.AddComponent<RoadHazard>();var c=go.GetComponentInChildren<Collider>()??go.AddComponent<BoxCollider>();var body=go.AddComponent<Rigidbody>();body.isKinematic=true;var mover=go.AddComponent<SideCrossingHazard>();mover.Configure(lane*EmergencyRoadGame.LaneWidth,left);spawned.Add(go);
        }
    ...
    public sealed class MotorRushHazard:MonoBehaviour
    {
        ...
        private IEnumerator Explode(bool preserveAfterPlayerHit)
        {
            ...
            var psGo=new GameObject("Motor Impact Burst",typeof(ParticleSystem));psGo.transform.position=transform.position;var ps=psGo.GetComponent<ParticleSystem>();var main=ps.main;main.duration=.45f;main.loop=false;main.startLifetime=new ParticleSystem.MinMaxCurve(.55f,1.05f);main.startSpeed=new ParticleSystem.MinMaxCurve(5f,9f);main.startSize=new ParticleSystem.MinMaxCurve(.25f,.55f);main.startColor=new ParticleSystem.MinMaxGradient(new Color(1f,.08f,.01f),new Color(1f,.85f,.08f));main.maxParticles=36;var emission=ps.emission;emission.rateOverTime=0;emission.SetBursts(new[]{new ParticleSystem.Burst(0,26)});var vfxMaterial=Resources.Load<Material>("EmergencyRoadVFX");if(vfxMaterial!=null)psGo.GetComponent<ParticleSystemRenderer>().sharedMaterial=vfxMaterial;ps.Play();var flash=new GameObject("Motor Explosion Flash",typeof(Light));flash.transform.position=transform.position;var light=flash.GetComponent<Light>();light.type=LightType.Point;light.range=9f;light.intensity=5f;light.color=new Color(1f,.28f,.03f);Destroy(flash,.18f);Destroy(psGo,1.5f);
    ...
    public static class EmergencyImpactVfx
    {
        ...
        {
            var go=new GameObject(name,typeof(ParticleSystem));go.transform.SetParent(parent,false);var ps=go.GetComponent<ParticleSystem>();var main=ps.main;main.loop=true;main.startLifetime=new ParticleSystem.MinMaxCurve(minimumLifetime,maximumLifetime);main.startSpeed=new ParticleSystem.MinMaxCurve(speed*.65f,speed*1.25f);main.startSize=new ParticleSystem.MinMaxCurve(size*.65f,size*1.25f);main.startColor=new ParticleSystem.MinMaxGradient(start,end);main.maxParticles=72;main.simulationSpace=ParticleSystemSimulationSpace.World;var emission=ps.emission;emission.rateOverTime=rate;var shape=ps.shape;shape.shapeType=ParticleSystemShapeType.Cone;shape.angle=18f;shape.radius=.18f;go.transform.localRotation=Quaternion.Euler(-90,0,0);var color=ps.colorOverLifetime;color.enabled=true;var gradient=new Gradient();gradient.SetKeys(new[]{new GradientColorKey(Color.white,0),new GradientColorKey(Color.white,1)},new[]{new GradientAlphaKey(1,0),new GradientAlphaKey(0,1)});color.color=gradient;var noise=ps.noise;noise.enabled=true;noise.strength=.22f;noise.frequency=.55f;noise.scrollSpeed=.35f;var material=Resources.Load<Material>("EmergencyRoadVFX");if(material!=null)go.GetComponent<ParticleSystemRenderer>().sharedMaterial=material;ps.Play();
        }
    }
}
