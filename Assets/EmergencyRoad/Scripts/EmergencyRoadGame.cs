...
namespace EmergencyRoad
{
    public sealed class EmergencyRoadGame : MonoBehaviour
    {
        ...
        internal bool ShouldSpawnObstacle(int sequence,bool crossroad)
        {
            ...
            // Each row picks a gap inside the authored range; speed and difficulty never affect this spacing.
            float safetyGap=VehicleLength*2f;float authoredMin=Settings!=null?Settings.minimumObstacleGap:36f;float authoredMax=Settings!=null?Settings.maximumObstacleGap:50f;float randomGap=Random.Range(Mathf.Min(authoredMin,authoredMax),Mathf.Max(authoredMin,authoredMax));nextObstacleDistance=generationDistance+Mathf.Max(safetyGap,randomGap);
            return true;
        }
    ...
    public sealed class EmergencyVehicleController : MonoBehaviour
    {
        ...
        {
            var pickup=other.GetComponentInParent<RoadPickup>();if(pickup!=null){game.AddCoin();pickup.Collect();return;}var impact=other.bounds.center-transform.position;if(other.GetComponentInParent<MotorRushHazard>()!=null){game.Crash(impact);return;}if(other.GetComponentInParent<RoadHazard>()==null)return;bool sideContact=Mathf.Abs(impact.x)>1.15f&&Mathf.Abs(impact.z)<1.65f;if(sideContact&&EmergencyRoadProfile.Current.sideCollisionEnabled){int side=impact.x>=0f?1:-1;lane=previousLane;targetX=lane*EmergencyRoadGame.LaneWidth;StartEdgeSqueeze(side);return;}game.Crash(impact);
        }
        private void OnCollisionEnter(Collision collision){if(collision.gameObject.GetComponentInParent<MotorRushHazard>()!=null||collision.gameObject.GetComponentInParent<RoadHazard>()!=null)game.Crash(collision.collider.bounds.center-transform.position);}
