...
namespace EmergencyRoad
{
    public static class EmergencyRoadBootstrap
    {
        ...
        private static void Compose(Scene scene)
        {
            ...
            if ((authoring != null && authoring.SceneKind == EmergencyRoadSceneKind.Menu) || scene.name == "Menu") EmergencyRoadMenu.Create(catalog);
            else EmergencyRoadGame.Create(catalog, authoring != null ? authoring.LaneWidth : catalog.laneWidth, catalog.roadLength > 1f ? catalog.roadLength : (authoring != null ? authoring.ChunkSpacing : 28f));
        }
    }
}
