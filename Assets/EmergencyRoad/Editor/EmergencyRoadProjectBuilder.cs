...
namespace EmergencyRoad.Editor
{
    ...
    public static class EmergencyRoadProjectBuilder
    {
        ...
        private static void CreateGroundStrip(Transform chunk,int side,EmergencyRoadCatalog catalog)
        {
            ...
            float grassWidth=catalog.gameplaySettings!=null?catalog.gameplaySettings.roadsideGrassWidth:42f;float grassInner=catalog.roadHalfWidth+1f;var grass=GameObject.CreatePrimitive(PrimitiveType.Cube);grass.name="Continuous Grass Ground";grass.transform.SetParent(group);grass.transform.localPosition=new Vector3(side*(grassInner+grassWidth*.5f),-.28f,0);grass.transform.localScale=new Vector3(grassWidth,.36f,catalog.roadLength+.08f);if(catalog.grassMaterial!=null)grass.GetComponent<Renderer>().sharedMaterial=catalog.grassMaterial;
        ...
        private static void CreateCrossroadExtensions(Scene scene,Transform chunk,EmergencyRoadCatalog catalog)
        {
            ...
            int radius=metrics!=null?Mathf.Max(1,Mathf.CeilToInt((metrics.rendererSize.z/catalog.roadLength-1f)*.5f)):1;float gap=Mathf.Max(0,(radius+.5f)*catalog.roadLength-crossHalfLength);if(gap>.03f)for(int side=-1;side<=1;side+=2){var connector=(GameObject)PrefabUtility.InstantiatePrefab(catalog.roadPrefabs[0],scene);connector.name=$"Main Road Connector {(side<0?"Back":"Forward")} - {gap:F2}m";connector.transform.SetParent(group);connector.transform.localPosition=new Vector3(0,-.006f,side*(crossHalfLength+gap*.5f));connector.transform.localScale=new Vector3(1,1,gap/catalog.roadLength);float grassWidth=catalog.gameplaySettings!=null?catalog.gameplaySettings.roadsideGrassWidth:42f;float grassInner=catalog.roadHalfWidth+1f;for(int xSide=-1;xSide<=1;xSide+=2){var grass=GameObject.CreatePrimitive(PrimitiveType.Cube);grass.name="Intersection Corner Grass - Exact Gap Fill";grass.transform.SetParent(group);grass.transform.localPosition=new Vector3(xSide*(grassInner+grassWidth*.5f),-.28f,side*(crossHalfLength+gap*.5f));grass.transform.localScale=new Vector3(grassWidth,.36f,gap+.04f);if(catalog.grassMaterial!=null)grass.GetComponent<Renderer>().sharedMaterial=catalog.grassMaterial;}}
